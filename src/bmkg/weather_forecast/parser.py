    ...
    for area_element in forecast_element.iterfind("area"):
        ...
        for parameter_element in area_element.iterfind("parameter"):
            ...
            if (
                ...
                parameter_id == "t" or parameter_id == "tmax" or parameter_id == "tmin"
            ):
        ...
        # forecast data received is for three days, each receiving 4 data. However, for
        # tmin, tmax, hmin, and hmax data, these four data are only given 1 each for
        # that day, therefore there are only 3 data for 3 days from tmin, tmax, hmin,
        # and hmax. Because the zip() function will stop at the smallest data length,
        # therefore we need to multiply the 3 data by 4 to get 12
        weathers = []
        for dt, weather, t, tmix, tmax, h, hmin, hmax, wd, ws in zip(
            parameters["datetime"],
            parameters["weather"],
            parameters["t"],
            chain.from_iterable([(val, val, val, val) for val in parameters["tmin"]]),
            chain.from_iterable([(val, val, val, val) for val in parameters["tmax"]]),
            parameters["hu"],
            chain.from_iterable([(val, val, val, val) for val in parameters["humin"]]),
            chain.from_iterable([(val, val, val, val) for val in parameters["humax"]]),
            parameters["wd"],
            parameters["ws"],
        ):
            weathers.append(Weather(dt, weather, t, tmix, tmax, h, hmin, hmax, wd, ws))
